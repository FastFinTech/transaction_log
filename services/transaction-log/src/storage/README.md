...
Manage the on-disk layout, file acquisition and metadata for transaction-log
streams. The provider creates stream directories and fresh pairs, opens files for
recovery, discovers logs, removes discarded ranges and persists checkpoints and
established configuration. These operations are implemented; application startup
...
| Type | Responsibility |
| --- | --- |
| [`StorageConfig`] | Own an immutable absolute root, resolved without filesystem changes. |
| [`StorageProvider`] | Construct paths and perform directory, file and metadata operations. |
| [`StorageError`] | Preserve the affected path and concrete I/O, JSON or size-limit failure. |

[Stream operations](crate::streams) own record ordering, recovery decisions and
checkpoint certification. [Clustering configuration](crate::clustering::configuration)
owns the established deployment value stored by this provider.

## Usage

Resolve the root, then await provider construction before requesting a fresh pair:

```no_run
use std::path::PathBuf;
use tokio::fs::File;
use transaction_log::{
    storage::{StorageConfig, StorageProvider},
    streams::LogFileId,
};
...
async fn create_pair(
    ...
    let storage = StorageProvider::new(StorageConfig::new(root)?).await?;
    Ok(storage.create_log_and_index(file_id).await?)
}
```

Construction preserves existing contents. Fresh-pair creation requires both files
to be absent; existing or partially created pairs go through recovery. Returned
handles are owned by the caller, which coordinates access and durability.

## Behavior and guarantees

### Configuration and ownership

Configuration anchors relative roots to the current directory once. Provider
construction creates all 4,096 stream bases before returning; it establishes
existence, not exclusivity or durable directory entries.
...
provider. Per-stream recovery is implemented by `StreamInitializer`; application startup
does not invoke it yet. There is no initialization flag or per-call readiness check;
...
`streams` component changes where files are found. The provider is the authoritative home for naming; shared construction keeps
readers, writers and recovery on the same paths.

</details>

### Maximum log discovery

`maximum_log_file(stream_id)` returns the highest canonical log, or `None` when
none exists. It does not prove a contiguous or valid stream. Concurrent tree
changes are excluded by the caller.
...
`open_log_for_validation` opens an existing log. `open_index_for_repair` opens or
creates its index. Both return read/write handles without append mode. Recovery
opens the log first, then establishes valid contents and append positions.
...
`create_log_and_index` creates missing parents and a new `(log, index)` pair at
byte zero. Existing files are preserved and rejected. Creation does not synchronize
files or directories, and a failure can leave partial creation.
...
| Operation | Contract |
| --- | --- |
| `read_checkpoint(stream_id)` | Read the published checkpoint with an inclusive 4 KiB limit; absence returns `None`. |
| `read_clustering_configuration()` | Read the published established configuration with the same limit; absence returns `None`. |
| `write_checkpoint(&checkpoint)` | Stage, synchronize and replace the supplied checkpoint. The caller certifies its covered records and index. |
| `write_clustering_configuration(&configuration)` | Stage, synchronize and replace supplied deployment metadata. The lifecycle caller enforces permanent identity and write-once establishment. |

Readers ignore `.pending` files. Writers serialize before I/O and publish by
rename. A failure after rename can leave the new document visible. Unix publication
synchronizes the immediate parent; Windows has no directory-durability guarantee.
Ancestors and covered log/index files require their owner's durability ordering.
...
[`EstablishedClusteringConfiguration`](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log/src/clustering/configuration/README.md#established-configuration)
...
`read_clustering_configuration().await` reads root `clustering.json`,
...
The [streams checkpoint module](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log/src/streams/README.md#stream-checkpoint-model) owns
...
`remove_log_and_index(first..=last)` removes an inclusive range from one stream,
accepting absent files. Bounds are checked before I/O; errors can follow partial
deletions. Checkpoint publication waits for successful cleanup.
...
Storage returns files and persists supplied metadata. The stream owner decides
which records are valid and ready; clustering and live-serving integration remain
planned.

<details>
<summary>Design and maintenance notes</summary>

The [streams module](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log/src/streams/README.md) owns one log/index pair exclusively,
...
The [record specification](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log-exports/src/record/README.md)
owns record framing; the provider preserves the reader's validation responsibilities.

</details>

## Performance

Path construction allocates owned paths without filesystem I/O. Discovery searches
the fixed-depth tree without retaining all file IDs; range removal retains one
path per affected parent so it can amortize directory synchronization. No provider
or recovery throughput has been measured.
...
design choice. Caches, shared scratch buffers, fixed-size formatting and caller-supplied output
buffers would add machinery without a demonstrated need.
...
<a id="verification-and-maintenance"></a>
...
```powershell
cargo test -p transaction-log --locked storage
cargo test -p transaction-log --release --locked storage
cargo clippy --workspace --all-targets --locked -- -D warnings
cargo fmt --all --check
cargo rustdoc -p transaction-log --bin transaction-log --locked -- -D warnings
```

Filesystem tests have platform-specific branches for Windows and Unix. The
...
explains executable examples in this binary crate.

<details>
<summary>Design and maintenance notes</summary>

Location and checkpoint tests are specified in their owning
[location module](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log/src/streams/location/README.md#validation) and
[streams module](https://github.com/FastFinTech/transaction_log/blob/main/services/transaction-log/src/streams/README.md#checkpoint-validation).

Configuration tests cover relative
...
Owned guards isolate stream cases while reusing the initialized directory layout.
Root-wide configuration tests use a separate metadata guard.
...
assigns distinct IDs across all consuming suites; each additional stream has its own guard. Independent parameter-loop cases use
separate guards, so allocation order and numeric IDs are not stable fixtures.

Each guard owns only its stream's contents. Its `Drop` removes files and nested
range directories, preserving the stream base. The guard outlives the
file handles and state that use it, with filesystem work completed before scope
exit. Copying its provider
...
lock file. Lock acquisition runs on a blocking worker. Shared fixture initialization must complete before use or cleanup because
cancellation does not stop spawned filesystem work.
